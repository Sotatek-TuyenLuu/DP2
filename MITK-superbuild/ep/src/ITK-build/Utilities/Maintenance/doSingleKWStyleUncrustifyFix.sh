...
${UNCRUSTIFYBIN} -c E:/D/MITK-superbuild/ep/src/ITK/Utilities/Maintenance/uncrustify_itk.cfg -l CPP -f ${file} | sed 's/  *##  */##/g' | sed 's/#  */#/g' > ${file}_uncrustify
...
KWSTYLE_EXECUTABLE-NOTFOUND -xml E:/D/MITK-superbuild/ep/src/ITK-build/Utilities/KWStyle/ITK.kws.xml -v -o E:/D/MITK-superbuild/ep/src/ITK/Utilities/KWStyle/ITKOverwrite.txt -gcc $file > ${KWERROS}
...
if [ -f ${KWERROS} ]; then
  ...
  if [ ${skipuncrustify} == "y" ]; then
    ${UNCRUSTIFYBIN} -c E:/D/MITK-superbuild/ep/src/ITK/Utilities/Maintenance/uncrustify_itk.cfg -l CPP -f ${file} -o ${KWERROS}_skip_uncrustify
  fi
